...
console.log("Emscripten version: D:/UE/Source/ue-4.27-html5-es3/Engine/Platforms/HTML5/Build/emsdk/emsdk-4.0.6/upstream/emscripten");
console.log("Emscripten configuration: D:/UE/Source/ue-4.27-html5-es3/Engine/Platforms/HTML5/Build/emsdk/emsdk-4.0.6/.emscripten");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_DuskUE', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202605120435, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : ['../../../DuskUE/DuskUE.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('DuskUE.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('DuskUE.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('DuskUE'+dataVariant+'.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('DuskUE.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('DuskUE.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('DuskUE'+dataVariant+'.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('DuskUE'+dataVariant+'.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
